...
def upload_file():
    ...
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'payment_data.xlsx')
    file.save(file_path)
...
def export_renewals():
    ...
    try:
        ...
        output_file = os.path.join(app.config['UPLOAD_FOLDER'], f'renewals_{year}{month_name}.xlsx')
...
def export_batch_pending_summary():
    ...
    try:
        ...
        output_file = os.path.join(app.config['UPLOAD_FOLDER'], f'batch_pending_summary_{year}.xlsx')
...
def export_selected_batch_pending():
    ...
    try:
        ...
        output_file = os.path.join(app.config['UPLOAD_FOLDER'], f'batch_{batch}_pending_{year}.xlsx')
        export_df.to_excel(output_file, index=False)
...
def export_payment_details():
    ...
    try:
        ...
        if all_data:
            ...
            output_file = os.path.join(app.config['UPLOAD_FOLDER'], f'payment_details_{name}.xlsx')
...
def export_rm_pending():
    ...
    try:
        ...
        output_file = os.path.join(app.config['UPLOAD_FOLDER'], f'rm_pending_{year}.xlsx')
...
def upload_nbt_file():
    ...
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'nbt_data.xlsx')
    file.save(file_path)
